...
client.on('messageCreate', async (message) => {
  if (message.content.toLowerCase() === 's!ping') {
    ...
    const exampleEmbed = {
      ...
      author: {
        name: 'StarrBot - Ping',
        icon_url: 'https://www.pngmart.com/files/16/Abstract-Gold-Star-PNG-Image.png',
...
message.reply(`**Saat(TR)**: *${new Date().toLocaleString('tr-TR', { timeZone: 'Europe/Istanbul' })}*`);
  }
});
...
client.on('messageCreate', async message => {
  if (message.content.startsWith('s!ban')) {
    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.reply('<:hata:1192466178061385728> **Kullanıcıları banlama yetkiniz yok.**');
    }

    // Botun BanMembers yetkisini kontrol et
    if (!message.guild.members.me.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.reply('**<:hata:1192466178061385728> Botun bu kullanıcıyı banlama yetkisi yok.**');
    }
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`**Banlanan Kullanıcı:** ${userToBan.tag}\n**ID:** ${userToBan.id}\n**Sebep:** ${reason}`)
        .setColor(0xFF0000)
        .setFooter({ text: `Banlayan Yetkili: ${message.author.tag}`, iconURL: message.author.displayAvatarURL() });
      await message.reply({ embeds: [embed] });
    } catch (error) {
      console.error('Ban işlemi sırasında bir hata oluştu:', error);
      await message.reply('<:hata:1192466178061385728> **Ban işlemi sırasında bir hata oluştu.**');
  }
  }
});
